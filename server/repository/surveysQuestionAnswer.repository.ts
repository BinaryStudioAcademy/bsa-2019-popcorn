...
class SurveysQuestionAnswerRepository extends Repository<SurveysQuestionAnswer> {
  async setQuestionAnswer(userId, optionId, questionId, value, next?) {
    try {
      ...
      const surveysQuestionOption = await getCustomRepository(SurveyQuestionOption).findOne({ id: optionId }); 
      const surveysQuestion = await getCustomRepository(SurveyQuestion).findOne({ id: questionId} );

      const prevAnswer = await this.findOne({user,  surveysQuestionOption, surveysQuestion });
      return prevAnswer
