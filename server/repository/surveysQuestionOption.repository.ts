...
class SurveysQuestionOptionRepository extends Repository<SurveysQuestionOption> {
  ...
  async getQuestionOptionByQuestionId(id: string, next?) {
    try {
      const surveysQuestion = await getCustomRepository(SurveysQuestionRepository).getSurveysQuestionById(id, next);
