...
class VotingOptionReactionRepository extends Repository<VotingOptionReaction> {
  async setVotingReaction(optionId: string, userId: string, isChosen: boolean, next?) {
    try {
      const votingOption = await getCustomRepository(VotingOptionRepository).findOne({ id: optionId });
      if(!votingOption)
