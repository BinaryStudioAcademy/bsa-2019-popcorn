...
class SurveysQuestionRepository extends Repository<SurveysQuestion> {
  ...
  async createSurveysQuestion(surveysQuestion: SurveysQuestion, next?, surveys?: Surveys) {
    try {
      if (surveys) 
        surveysQuestion.surveys = surveys;

      await Promise.all(surveysQuestion.surveysQuestionOption.map(option => 
        getCustomRepository(SurveysQuestionOptionRepository).createQuestionOption(option, next)
      ));
  ...
  async getSurveysQuestionBySurveysId(id: string, next?) {
    try {
      const surveys = await getCustomRepository(SurveysRepository).getSurveysById(id, next);
  ...
  async updateSurveysQuestionById(id: string, surveysQuestion: SurveysQuestion, next?) {
    try {
  ...
  async deleteSurveysQuestionById(id: string, next?) {
    try {
      ...
      if(!surveysQuestion) {
        return next({status: 404, message: 'Surveys Option is not found'}, null);
      }
