...
export const createSurveys = async (body: IRequest, next): Promise<Surveys> => {
  return await getCustomRepository(SurveysRepository).createSurveys(body.userId, body.surveys, body.surveysQuestion, next);
};
...
export const deleteSurveysById = async (id: string, next): Promise<{ }> => {
  return await getCustomRepository(SurveysRepository).deleteSurveysById(id, next);
}

export const updateSurveysById = async (id: string, surveys: Surveys, next): Promise<Surveys> => {
  return await getCustomRepository(SurveysRepository).updateSurveysById(id, surveys, next);
}

export const getSurveysByUserId = async (id: string, next): Promise<Surveys[]> => {
  return await getCustomRepository(SurveysRepository).getSurveysByUserId(id, next);
}
