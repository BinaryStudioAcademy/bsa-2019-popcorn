...
const FilmInput: React.FC<IInputProps> = ({ saveMovie, clearSearch, movie, deleteFilmInput, alreadySearch, movieList, fetchFilms, }) => {
    ...
    return (
        <div
                ...
                <div className="modal modal-top">
                    {movieList && movieList.length > 0 ?
                        movieList.map((searchedMovie, index) =>
                            <div className="hover"
                                key={index}
                                onClick={() => changeTitle({ movieId: movie.id, title: searchedMovie._source.title })}>
                                {searchedMovie._source.title}</div>)
            ...
            <textarea
                maxLength={140}
                value={comment}
                onChange={(e) => { setComment(e.target.value); saveMovie({ ...movie, title, comment }); }}
